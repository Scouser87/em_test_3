...
float operator^(const vec2 &a, const vec2 &b)
{ 
    return (a.n[VX] * b.n[VY] - b.n[VX] * a.n[VY]); 
}
...
mat3 rotation2D(float angleRad) 
{
...
float angle(vec2 v)
{
	static vec2 norm=vec2(1,0).normalize();
	static float ang,d;
	if(v==norm) return 0;
	ang= acos((v*norm)/(lenght(v)));
	d=v^norm;
	if(d>0)
	{
		ang=2*M_PI - ang; 
...
float angle(vec2 V1,vec2 V2)
{
	...
	if(V1==V2) return 0;
	if(V1==vec2(0,0)) V1=vec2(1,0);
	if(V2==vec2(0,0)) V2=vec2(1,0);
	
	ang= acos((V1*V2)/(lenght(V1)*lenght(V2)));
	d=V1^V2;
	if(d<0)
	{
		ang=2*M_PI - ang; 
...
bool IsInCircle(vec2 &p,vec2 &pos,float r)
{
	...
	if(abs(lenght(p,pos)) <=r )return true;
	return false;
...
{
	if( abs(lenght(p1,p2)) < r1+r2 )return 1;
	return 0;
}
